   ...
   .directive('dashboard', ['WidgetModel', 'WidgetDefCollection', '$modal', 'DashboardState', '$log', function (WidgetModel, WidgetDefCollection, $modal, DashboardState, $log) {
      return {
         ...
         controller: ['$scope', '$attrs', function (scope, attrs) {
            ...
            // add this to enable callback from demo.js
            // scope.$eval(attrs.axCallback);
         ...
         link: function (scope, attrs) {
            ...
            // console.log(attrs.attr('ax-callback'));

            scope.axCallback = function () {
               scope.$eval(attrs.attr('ax-callback'));
            };
...
angular.module("ui.dashboard").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("template/dashboard.html",
    ...
    "      <button class=\"btn btn-warning\" ng-click=\"resetWidgetsToDefault()\">Default Widgets</button>\n" +
    "\n" +
    "      <button ng-if=\"options.storage && options.explicitSave\" ng-click=\"options.saveDashboard()\" class=\"btn btn-success\"\n" +
    "              ng-disabled=\"!options.unsavedChangeCount\">{{ !options.unsavedChangeCount ? \"all saved\" : \"save changes (\"\n" +
    ...
    "                        ng-if=\"!options.hideWidgetSettings\"></span>\n" +
    "                  <span ng-repeat=\"icon in widget.attrs.toolbar.icons\" ng-click=\"axCallback()\"\n" +
    "                        class=\"{{icon}}\" ng-if=\"!options.hideWidgetSettings\"></span>\n" +
    "               </h3>\n" +
    "            </div>\n" +
